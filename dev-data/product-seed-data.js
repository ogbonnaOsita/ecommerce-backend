...
async function seedDB() {
    ...
    try {
      ...
      for (let i = 0; i < titlesArr.length; i++) {
        let prod = new Product({
          sku: faker.helpers.replaceSymbols('***-****-**-****'),
          title: titlesArr[i],
          price: faker.commerce.price({ min: 10, max: 300 }),
  ...
  await seedProducts(travelbags_titles, travelbags_imgs, 'Travel Bags');
